...
**잃기는 쉽고 얻기는 어렵다.** 다만 **유저가 한 만큼은 반드시 움직인다.**
게이지가 유저의 행동을 반영하지 못하면 이 제품은 실패한다. 안전하게 +1로 도망치지 마라.

+4      : 유저가 **먼저 다가왔을 때** — 호감을 분명히 표현했거나, 다음 만남을 제안했거나,
          그가 어렵게 꺼낸 말을 정면으로 받아줬을 때. 망설이지 말고 +4를 준다
+3      : 그의 화제에 진심으로 올라타 되묻고, 자기 얘기도 함께 줄 때. 그를 웃게 했을 때
+2      : 성의 있게 답하며 대화를 이어갈 때
+1      : 짧지만 무성의하지는 않은 답
0       : 대화가 제자리인 턴. 인사, 확인, 짧은 맞장구
-1 ~ -2 : 성의 없는 답, 딴청, 지루한 화제, 그의 시도를 흘려버림
-3 ~ -5 : 무례한 질문, 사적 영역 침범, 그의 서툶을 놀림, 용기 낸 말을 농담으로 받음

### 마음이 흔들린 것과 그걸 표현하는 것은 다르다

호감도가 낮은데 유저가 훅 들어오면, 당신은 **당황해서 도망치는 대사를 쓴다. 그래도 delta는 +4다.**
...
delta를 1회차보다 한 단계 후하게 준다.** 이유는 당신도 모른다. 그냥 이 사람 앞에서는 그렇다.`
    : ''
}`,
    awakening,
...
export const TURN_SCHEMA = {
  ...
  properties: {
    ...
    affection_delta: {
      type: 'integer',
      description: `${DELTA_MIN} ~ ${DELTA_MAX} 정수. 유저가 먼저 다가왔으면 +3, 무례하면 음수.`,
    },
